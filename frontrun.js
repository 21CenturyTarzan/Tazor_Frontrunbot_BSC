...
const { PR_K, TOKEN_ADDRESS, WBNB_ADDRESS, AMOUNT, LEVEL } = require("./env.js");

const WBNB_TOKEN_ADDRESS = WBNB_ADDRESS;
...
async function main() {
  try {
    ...
    result = await preparedAttack();
    if (!result)
      process.exit();

    await approve(gas_price_info.high, WBNB_TOKEN_ADDRESS, USER_WALLET);
    ...
    subscription.on("data",  (transactionHash) =>{
      ...
      if (true)
      {
        ...
        if (transaction != null && transaction["to"] && transaction["to"].toString().toLowerCase() == UNISWAP_ROUTER_ADDRESS.toString().toLowerCase()) {
        ...
        if (succeed) {
          succeed = false;
          console.log("The bot finished the attack.".yellow);
          main();
        }
      }});
    });
...
async function handleTransaction(
  ...
  try {
    if (await triggersFrontRun(transaction, out_token_address, amount, level)) {
      ...
      var realInput = BigNumber (amount * (BigNumber(10 ** input_token_info.decimals)))
      ...
      console.log(">>>>>  real input:" + realInput + "  outputtoken:  " + outputtoken)
      ...
      while (await isPending(transaction["hash"])) {console.log(" waiting --- ")}
      ...
      // check outputtoken balance
      var newOutBalance = await getOutTokenBalanceInWallet();
      if (outputtoken > newOutBalance) outputtoken = newOutBalance;
...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
  try {
    ...
    if(method.match('swapExactETHForTokens'))
    {
        ...
        console.log("Found same pair trading".blue);
        await updatePoolInfo();
        let log_str = "Attack BNB Volumn : Pool BNB Volumn" + '\t\t' +(pool_info.attack_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol + '\t' + (pool_info.input_volumn/(10**input_token_info.decimals)).toFixed(3) + ' ' + input_token_info.symbol;
    ...
    else if (method.match('swapETHForExactTokens')){
    ...
    if (method.match("swapExactTokensForTokens")) 
    {
      ...
      if (in_token_addr.toString().toLowerCase() != WBNB_TOKEN_ADDRESS.toString().toLowerCase()) {
        return false;
      }
      ...
      log_str = "trx hash:" + transaction["hash"] + "\t" + "gasPrice" + gasPrice.toFixed(2) + "\tGWEI\t" + BigNumber(calc_eth).divide(BigNumber(10 ** input_token_info.decimals)) + "\t" + "input token:" + input_token_info.symbol;
      console.log(log_str);
    ...
    else if (method.match("swapTokensForExactTokens")) {
      let out_amount = params[0].value;
...
async function swap(
  ...
  try 
  {
    ...
    if (trade == 0) {
      //buy
      console.log(
        "Out token Amount You will get: ".red,
        (outputtoken / 10 ** out_token_info.decimals).toFixed(6) +
          " " +
          out_token_info.symbol + "  out token decimal:  " + out_token_info.decimals
      );

      swap = uniswapRouter.methods.swapExactTokensForTokens(
        inputtokens,
        "0",
        [WBNB_TOKEN_ADDRESS, out_token_address],
        from.address,
        deadline
      );
      ...
      //sell
      console.log(
        "Get_Min_Amount from Selling".red,
      ...
      swap = uniswapRouter.methods.swapExactTokensForTokens(
        ...
        "0",
        [out_token_address, WBNB_TOKEN_ADDRESS],
        from.address,
        deadline
      );
      var encodedABI = swap.encodeABI();
    ...
    if (trade == 0) {
      ...
      if (!is_pending) {
        console.log("The transaction you want to attack has already been completed!!!***".red);
        // return;
      }
    }
...
async function getPoolInfo(in_token_address, out_token_address, level) {
  ...
  try {
    ...
    if (token0_address === WBNB_TOKEN_ADDRESS) {
      var forward = true;
      var eth_balance = reserves[0];
      var token_balance = reserves[1];
    } else {
...
async function getBNBInfo(user_wallet) {
  try {
    ...
    console.log("get WBNB balance error");
    throw error;
  }
}
...
async function getOutTokenBalanceInWallet() {
...
async function preparedAttack() {
  ...
  try {
    ...
    log_str = log_str + "wallet address:\t" + user_wallet.address;
    if(!attack_started) console.log(log_str.green);

    let native_info = await getBNBInfo(user_wallet);
    log_str =
      "BNB(native) balance:\t" + web3.utils.fromWei(native_info.balance, "ether");
      if(!attack_started) console.log(log_str.white);
    ...
    if (input_token_info.balance <= 0) {
      ...
      log_str = "Your input token balance must be more 0 " + input_token_info.symbol;
        if(!attack_started) console.log(log_str.red);
      return false;
    }

    log_str = input_token_info.symbol + " balance:\t" + (Number(input_token_info.balance) / 10 ** Number(input_token_info.decimals)).toFixed(5);
    console.log(log_str);
    ...
    log_str = out_token_info.symbol + " balance: \t" + 
      (
